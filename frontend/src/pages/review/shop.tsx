...
import { GetShops, GetCategories, GetReviewsByShopId } from
...
const Shop: React.FC = () => {
 ...
 try {
 const reviewResponse = await GetReviewsByShopId(shopId); // Fetch reviews by shop_id
 const reviews: ReviewInterface[] = reviewResponse.data;
 
 if (reviews.length === 0) return 0;
 const totalScore = reviews.reduce((acc, review) => acc + (review.Score ??
0), 0);
 ...
 <div>
 {contextHolder}
 <p style={{ fontFamily: 'Itim, sans-serif', fontSize: '32px', textAlign:
'center' }}>รีวิวร้านค้า</p>
 <div>
...
/>}
 ...
 <Rate disabled value={shop.ID !== undefined ? 
ratings[shop.ID] : 0} allowHalf />
 <p>({shop.ID !== undefined && ratings[shop.ID] !== 
undefined ? (Math.round(ratings[shop.ID] * 10) / 10).toFixed(1) : 0} star)</p>
 </>
 }
 />
 </Card>
